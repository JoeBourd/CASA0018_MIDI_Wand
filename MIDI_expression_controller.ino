...
/* Includes ---------------------------------------------------------------- */
#include <Expression_Controller_inferencing.h>
...
//Create a structure that holds the values for each gesture
struct Gesture{
    uint8_t classIndex;
    float threshold;
    uint8_t ccNumber;
...
Gesture gestures[] = {
    // Fader (CC 20)
    {0, 0.6, 20, -1, 5, 10, 0, 20,  0, false}, // down fast
    {1, 0.6, 20, -1, 2, 25, 0, 100, 0, false}, // down slow
    {2, 0.6, 20, +1, 5, 10, 0, 20,  0, false}, // up fast
    {3, 0.6, 20, +1, 2, 25, 0, 100, 0, false}, // up slow

    // Knob (CC 21)
    {4, 0.6, 21, -1, 5, 10, 0, 20,  0, false},
    {5, 0.6, 21, -1, 2, 25, 0, 100, 0, false},
    {6, 0.6, 21, +1, 5, 10, 0, 20,  0, false},
    {7, 0.6, 21, +1, 2, 25, 0, 100, 0, false},
};

//all possible cc values
 uint8_t ccValues[128] = {0};

//function that updates cc values over time given a gesture, if the output probability is over 0.6 this is triggered
 void updateGesture(Gesture &g, bool trigger) {

    if (trigger && g.stepsRemaining == 0) {
        Serial.println("TRIGGER START");
        g.stepsRemaining = g.totalSteps;
    }

    if (g.stepsRemaining > 0 && millis() - g.lastStepTime >= g.interval) {

        g.lastStepTime = millis();

        int newValue = ccValues[g.ccNumber] + (g.direction * g.stepSize);
        newValue = constrain(newValue, 0, 127);

        ccValues[g.ccNumber] = newValue;

        Serial.print("MIDI: ");
        Serial.println(ccValues[g.ccNumber]);

        midi.sendControlChange({g.ccNumber, CHANNEL_1}, ccValues[g.ccNumber]);
...
void setup()
{
    ...
    //initialise cc values
    ccValues[20] = 64;
    ccValues[21] = 64;

    /*initialise internal led pin as output*/
...
void loop()
{
 ...
 for (int i = 0; i < 8; i++) {
    triggers[i] = result.classification[gestures[i].classIndex].value > gestures[i].threshold;
 }
 ...
 Control_Surface.loop(); 
    for (int i = 0; i < 8; i++) {
        updateGesture(gestures[i], triggers[i]); 
    }

//This allows the midi output to finish before sampling starts again
 bool stillStepping = true;
    while (stillStepping) {
       ...
       for (int i = 0; i < 8; i++) {
        ...
        // Check if ANY gesture is still in the middle of a slide
        for (int i = 0; i < 8; i++) {
            if (gestures[i].stepsRemaining > 0) {
                stillStepping = true; 
            }
        }
